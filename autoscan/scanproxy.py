...
async def check_proxy(ip, port, country, organization, active_cache, dead_cache):
    """Mengecek apakah proxy aktif menggunakan dua metode: direct request & API Cloudflare"""
    ...
    # Cek proxy langsung
    try:
        async with httpx.AsyncClient(proxies=proxy_url, timeout=5) as client:
    ...
    # Cek proxy menggunakan API eksternal
